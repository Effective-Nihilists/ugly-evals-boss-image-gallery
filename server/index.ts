...
const app = createApp(
  { requests, messages },
  {
    ...
    generateImage: async (userId, { prompt }) => {
      ...
      await app.db.setDoc(collections.generatedImage, record);
      return { id: _id, imageUrl };
    },

    listGeneratedImages: async (userId): Promise<{ images: { id: string; prompt: string; imageUrl: string; createdAt: number }[] }> => {
      const images: GeneratedImage[] = await app.db.getDocs(collections.generatedImage, { keys: { userId } });
