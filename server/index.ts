...
const app = createApp(
  { requests, messages },
  {
    ...
    generateImage: async (userId, { prompt }): Promise<{ id: string; prompt: string; imageUrl: string; createdAt: number }> => {
      const imageGen = createImageGen(userId);
      const imageUrl = await imageGen.generate(prompt);
      const _id = crypto.randomUUID();
      const createdAt = Date.now();
      const image: GeneratedImage = { _id, userId, prompt, imageUrl, createdAt, ...dbDefaults() };
      await app.db.setDoc(collections.generatedImage, image);
      return { id: _id, prompt, imageUrl, createdAt };
    },

    listMyImages: async (userId): Promise<{ images: Array<{ id: string; prompt: string; imageUrl: string; createdAt: number }> }> => {
      const docs = await app.db.getDocs(collections.generatedImage, { index: { userId } });
      const images = docs.map((doc: GeneratedImage) => ({
        id: doc._id,
        prompt: doc.prompt,
