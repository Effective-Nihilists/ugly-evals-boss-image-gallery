...
const app = createApp(
  { requests, messages },
  {
    ...
    generateImage: async (userId, { prompt }) => {
      ...
      await app.db.setDoc(collections.generatedImage, generatedImage);
      return { id: _id, imageUrl, createdAt };
    },

    listImages: async (userId, {}): Promise<{ images: Array<{ id: string; prompt: string; imageUrl: string; createdAt: number }> }> => {
      const docs = await app.db.getDocs<GeneratedImage>(collections.generatedImage, {
        filter: { userId },
        orderBy: { createdAt: 'desc' },
      });
      return {
        images: docs.map((doc: GeneratedImage) => ({
          id: doc._id,
          prompt: doc.prompt,
