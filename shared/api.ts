...
export const requests = defineRequests({
  ...
  generateImage: authReq({
    ...
    output: z.object({ id: z.string(), prompt: z.string(), imageUrl: z.string() }),
    rateLimit: { max: 10, window: 60 },
  }),

  // Image gallery — list caller's previously generated images
  listMyImages: authReq({
    input: z.object({}),
    output: z.object({ images: z.array(z.object({ id: z.string(), prompt: z.string(), imageUrl: z.string(), created: z.number() })) }),
  }),
